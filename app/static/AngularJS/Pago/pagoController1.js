...
registrationModule.controller("pagoController", function($scope, $http, $interval, uiGridGroupingConstants, utils, uiGridConstants, $filter, $rootScope, localStorageService, alertFactory, pagoRepository, stats, $window, $routeParams) {
        ...
        if ($scope.currentEmployee == null) {
            $scope.currentEmployee = 71; //25:1;
            $scope.idEmpresa = 1;
        } else {
            $scope.idUsuario = $scope.currentEmployee;
            $scope.idEmpresa = 1;
        }
        ...
        $scope.menuurl = $routeParams.idmenu;
        ...
        ****************************************************************************************************************/
        ...
        $scope.init = function() {
            ...
            if ($rootScope.lgentra == 1)
            {
                $rootScope.lgentra = $rootScope.lgentra + 1;
                if (($scope.idUsuario == 341) || ($scope.idUsuario == 342))
                {

                $scope.verlote=true;
                $scope.vermodal=true;
                $scope.vermonitor = false;
            ...
            if (($scope.menuurl == "undefined")||($scope.menuurl == 1)||($scope.menuurl == undefined))
            {
                $scope.verlote=true;
                $scope.vermodal=true;
                $scope.vermonitor = true;
                $scope.verbusqueda = false;
              
            }
            else if($scope.menuurl == 2)
            {
                $scope.verlote=true;
            ...
            if (getParameterByName('idOperacion') != '') {
                ...
                pagoRepository.getLotes($scope.idEmpresa, $scope.currentEmployee, 0, idLote)
        ...
        var GetEmpleado = function() {
            if (!($('#lgnUser').val().indexOf('[') > -1)) {
                ...
                if (($('#lgnUser').val().indexOf('[') > -1) && !localStorageService.get('lgnUser')) {
                    if (getParameterByName('employee') != '') {
                        $scope.currentEmployee = getParameterByName('employee');
                        return;
                    } else {
                        alert('Inicie sesión desde panel de aplicaciones.');
                        window.close();
        ...
        $scope.ObtieneLotes = function(newLote) //borraLote, 0 para borrar lotes sin relacion, 1 para conservarlos
            {
                var date = new Date();
                pagoRepository.getLotes($scope.idEmpresa, $scope.currentEmployee, 0, 0)
                    .then(function successCallback(data) {
                            var EsPagoDirecto = 0;
        ...
        $scope.LlenaIngresos = function() {
            pagoRepository.getIngresos($scope.idEmpresa, $scope.idLote)
                .then(function successCallback(response) {
                    $scope.bancoIngresos = [];
                    //lleno ingresos sin
        ...
        ****************************************************************************************************************/
        ...
        $scope.BuscarTesoreria = function() {
            ...
            $scope.gridtesoreriaoptions.columnDefs = [
                ...
                { name: 'generaTXT', displayName: 'TXT', width: '5%', cellTemplate: '<div ng-show="(row.entity.estatus==3)||(row.entity.estatus==4)"><button class="btn btn-default" ><span class="glyphicon glyphicon-list-alt"  ng-click="grid.appScope.GenerarArchivoBtn(row.entity.idEmpresa, row.entity.idLotePago)"></span></button></div>' },
        ...
        $scope.ActualizarCartera = function() {
            $scope.isDisabled = true;
            pagoRepository.UpdateCartera($scope.idEmpresa).then(function successCallback(response) {
                console.log(response.data);
                if (response.data.length == 0) {
                    $scope.isDisabled = false;
                }
        ...
        $scope.BuscarLotesxFecha = function(fechaini, fechafin) {
            ...
            pagoRepository.getLotesxFecha($scope.idEmpresa, $scope.idUsuario, fecha_ini, fecha_fin,0)
         ...
         $scope.BuscarLotesxFechaTesoreria = function(fechaini, fechafin) {
            ...
            pagoRepository.getLotesxFecha($scope.idEmpresa, $scope.idUsuario, fecha_ini, fecha_fin,3)
        ...
        var getCarteraCallback = function(data, status, headers, config) {
            ...
            var newLote = { idLotePago: '0', idEmpresa: $scope.idEmpresa, idUsuario: $scope.currentEmployee, fecha: '', nombre: $scope.formData.nombreLoteNuevo, estatus: 0 };
        ...
        var ConfiguraGrid = function() {
            ...
            $scope.idEmpleado = $scope.currentEmployee;
            $scope.gridOptions = {
        ...
        aggregation.rendered = aggregation.value
                            ...
                            if (colDef.name == 'referencia') {
...

                                {
                                    if (newValue.length > 30) {
                                                                    alertFactory.warning('La referencia no puede tener más de 16 caracteres');
                                ...
                                {
                                   if (newValue.length > 30) {
                                                                    alertFactory.warning('La referencia CIE no puede tener más de 20 caracteres');
                                                                    rowEntity.referencia = oldValue;
                                                                }} 
                                }                                
            ...
            $scope.VerDocumento = function(lote) {
                 ...
                 pagoRepository.getPdf(lote.polTipo, lote.annio,lote.polMes,lote.polConsecutivo,$scope.idEmpresa).then(function(d) {
                            //Creo la URL
        ...
        var guardaValida = function(negativos, saldo, opcion, valor) {
            ...
            if (saldo <= 0) {
                ...
                if (rows.length == 0 && $scope.verbusqueda == false) {
                ...
                if (pasaxCIE)
                    ...
                    if ((pasaxegresos) && (opcion != 2) && (opcion != 3) && (pasaxbancoDestino))
                    {
                        ...
                        pagoRepository.getPagosPadre($scope.idEmpresa, $scope.currentEmployee, $scope.formData.nombreLoteNuevo, $scope.idLotePadre, EsPagoDirecto, ($scope.grdApagar).toFixed(2))
                            .then(function successCallback(response) {
                                ...
                                rows.forEach(function(row, i) {
                                    ...
                                    var numcuentaDestino = row.cuentaDestino.substring(primerparentesis + 1, lonbancodestino-1)
                                    elemento.pad_bancoDestino = numcuentaDestino;
                                    array.push(elemento);
                                    count = count + 1;
                                });
                                ...
                                pagoRepository.setDatos(array, $scope.currentEmployee, $scope.idLotePadre, jsIngresos, jsTransf, $scope.caja, $scope.cobrar, jsEgresos, ($scope.estatusLote == 0) ? 1 : 2)
                                    .then(function successCallback(response) {
                ...
                else
                {
                        ...
                        { //aprobacion
                            ...
                            var count = 1;
                                rows.forEach(function(row, i) {
                                    ...
                                    var lonbancodestino = row.cuentaDestino.length;
                                    var primerparentesis = row.cuentaDestino.indexOf("(",0) 
                                    var numcuentaDestino = (primerparentesis > -1)? row.cuentaDestino.substring(primerparentesis + 1, lonbancodestino-1) : row.cuentaDestino                                    
                                    elemento.pad_bancoDestino = numcuentaDestino;
                                    array.push(elemento);
                                     count = count + 1;
                                });

                                // pagoRepository.setDatosAutoriza(array, $scope.currentEmployee, $scope.idLotePadre)
                                //     .then(function successCallback(response) {
                                //         alertFactory.success('Se guardaron los datos.');
                                //       }, function errorCallback(response) {
                                //         alertFactory.error('Error al guardar Datos');
                                ...
                                pagoRepository.setAprobacion(1, valor, $scope.idEmpresa, $scope.idLotePadre, $scope.currentEmployee, $scope.idAprobador, $scope.idAprobacion, $scope.idNotify, $scope.formData.Observacion)
                                    .then(function successCallback(response) {
                                        if (valor == 3) {
                    ...
                    if (opcion == 3) { //aprobacion
                        pagoRepository.setAplicacion($scope.idEmpresa, $scope.idLotePadre, $scope.currentEmployee)
                            .then(function successCallback(response) {
                                if (valor == 3) {
                                    alertFactory.success('Se aprobo el lote con exito');
                                    $('#btnAprobar').button('reset');
        ...
        ************************************************************************************************************/
        ...
        $scope.GenerarArchivo = function(varidempresa, varidlote,dtfechaini,dtfechafin) {
         ...
         $scope.aplicaLoteDirecto = function(idlote,dtfechaini,dtfechafin) {
           $('#btn_AplicarDirecto').button('loading');
            pagoRepository.setAplicacionDirecta(idlote.idEmpresa, idlote.idLotePago, $scope.currentEmployee).then(function successCallback(response) {
                console.log(response.data);
                if (response.data.length == 0) {
                    $scope.isDisabled = false;
                }
         ...
         $scope.borraLote = function(Lote,dtfechaini,dtfechafin) {
                ...
                $scope.isDisabled = true;
                    //pagoRepository.setAplicacionDirecta($scope.idEmpresa, idlote, $scope.currentEmployee).then(function successCallback(response) {
        ...
        $scope.EnviaAprobacion = function() {
            ...
            pagoRepository.setSolAprobacion(1, 8, $scope.idEmpresa, $scope.idLotePadre)
                .then(function successCallback(response) {
